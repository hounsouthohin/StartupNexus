...
export const postService = {
  getAll: async (authorId: string, page: number = 1, pageSize: number = 20): Promise<SerializedPost[]> => {
    const items = await prisma.post.findMany({ where: { authorId }, select: { id: true, title: true, excerpt: true, status: true, categoryId: true, createdAt: true }, orderBy: { createdAt: 'desc' }, take: pageSize, skip: (page - 1) * pageSize })
    return items.map(item => ({ ...item, createdAt: item.createdAt.toISOString() })) as SerializedPost[]
  },

  getPublished: async (): Promise<SerializedPost[]> => {
    const items = await prisma.post.findMany({ where: { status: 'published' }, select: { id: true, title: true, excerpt: true, status: true, categoryId: true, createdAt: true }, orderBy: { createdAt: 'desc' }, take: 50, skip: 0 })
    return items.map(item => ({ ...item, createdAt: item.createdAt.toISOString() })) as SerializedPost[]
  },
  ...
  getAllWithRelations: async (authorId: string): Promise<SerializedPost[]> => {
    const items = await prisma.post.findMany({ where: { authorId }, select: { id: true, title: true, excerpt: true, status: true, categoryId: true, createdAt: true, category: { select: { id: true, name: true } } }, orderBy: { createdAt: 'desc' }, take: 20, skip: 0 })
    return items.map(item => ({ ...item, createdAt: item.createdAt.toISOString() })) as SerializedPost[]
  },

  getByIdWithRelations: async (authorId: string, id: string): Promise<SerializedPost> => {
    const item = await prisma.post.findFirst({ where: { id, authorId }, select: { id: true, title: true, excerpt: true, status: true, categoryId: true, createdAt: true, category: { select: { id: true, name: true } } } })
    if (!item) notFound()
  ...
  getPublicByIdWithRelations: async (id: string): Promise<SerializedPost> => {
    const item = await prisma.post.findUnique({ where: { id }, select: { id: true, title: true, excerpt: true, status: true, categoryId: true, createdAt: true, category: { select: { id: true, name: true } } } })
    if (!item) notFound()
    return (item => ({ ...item, createdAt: item.createdAt.toISOString() }))(item) as SerializedPost
  },
