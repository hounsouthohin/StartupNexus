...
export default function RecipeEditClient({ item, categoryOptions }: RecipeEditClientProps) {
  ...
  return (
    <main className="container mx-auto p-6 max-w-xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Modifier Recipe</h1>
      </div>

      {error && <p className="mb-4 text-sm text-red-600 bg-red-50 px-4 py-2 rounded">{error}</p>}

      <form action={updateRecipe.bind(null, item.id)} className="space-y-4 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <div>
        ...
        <div>
          ...
          <input
            ...
            id="slug"
            name="slug"
            defaultValue={item.slug != null ? String(item.slug) : ''}
            className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        <div>
