...
export const categoryService = {
  ...
  getAllWithRelations: async (authorId: string): Promise<SerializedCategory[]> => {
    const items = await prisma.category.findMany({ where: { authorId }, select: { id: true, name: true, createdAt: true, posts: { select: { id: true, title: true, excerpt: true } } }, orderBy: { createdAt: 'desc' }, take: 20, skip: 0 })
    return items.map(item => ({ ...item, createdAt: item.createdAt.toISOString() })) as SerializedCategory[]
  },

  getByIdWithRelations: async (authorId: string, id: string): Promise<SerializedCategory> => {
    const item = await prisma.category.findFirst({ where: { id, authorId }, select: { id: true, name: true, createdAt: true, posts: { select: { id: true, title: true, excerpt: true } } } })
    if (!item) notFound()
