...
export default function BlogClient({ items }: BlogClientProps) {
  ...
  const filtered = list
    ...
    .filter(item => !query || ['title', 'excerpt']
  ...
  return (
    <main className="container mx-auto p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/dashboard/new" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium">
          Nouveau
        </Link>
      </div>
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end gap-2">
                      <Link href={`/dashboard/${item.id}/edit`} className="px-3 py-1 text-xs font-medium text-blue-600 border border-blue-600 rounded hover:bg-blue-50">
                        Modifier
                      </Link>
                    </div>
                  </td>
