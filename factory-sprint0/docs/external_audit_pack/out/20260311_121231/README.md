...
Avec export Qdrant des standards actifs:

```powershell
powershell -ExecutionPolicy Bypass -File docs/external_audit_pack/build_pack.ps1 -ExportQdrant
```

Cela ajoute `qdrant_standards_snapshot.json` dans le dossier `out/<timestamp>` si Qdrant est accessible.
