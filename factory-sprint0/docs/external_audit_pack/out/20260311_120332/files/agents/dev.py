...
def dev_agent(
    ...
    def _prebuild_gates(files_dict: dict) -> tuple:
        ...
        if _usestate_violations:
            _warn_msg = (
                "USE_STATE TYPED WARNING — NON BLOQUANT\n"
            ...
            logger.warning(f"[PREBUILD WARN use_state_typed] {len(_usestate_violations)} fichier(s)")
            messages.append(HumanMessage(content=f"[PREBUILD WARNING:use_state_typed]\n{_warn_msg}"))
        ...
        if _app_router_violations:
            def _suggest_fix(fp_str: str) -> str:
                ...
                return str(_p.parent / _p.stem / "page.tsx")
            _warn_msg = (
                "APP ROUTER CONVENTION WARNING — NON BLOQUANT\n"
            ...
            logger.warning(f"[PREBUILD WARN app_router_convention] {len(_app_router_violations)} fichier(s)")
            messages.append(HumanMessage(content=f"[PREBUILD WARNING:app_router_convention]\n{_warn_msg}"))
        ...
        if _api_naming_violations:
            ...
            _warn_msg = (
                "APP ROUTER API NAMING WARNING — NON BLOQUANT\n"
            ...
            logger.warning(f"[PREBUILD WARN app_router_api_naming] {len(_api_naming_violations)} fichier(s)")
            messages.append(HumanMessage(content=f"[PREBUILD WARNING:app_router_api_naming]\n{_warn_msg}"))
        ...
        if _auth_violations:
            ...
            _warn_msg = (
                "AUTH WRAPPING ROUTE WARNING — NON BLOQUANT\n"
            ...
            logger.warning(f"[PREBUILD WARN auth_wrapping_route] {len(_auth_violations)} fichier(s)")
            messages.append(HumanMessage(content=f"[PREBUILD WARNING:auth_wrapping_route]\n{_warn_msg}"))
