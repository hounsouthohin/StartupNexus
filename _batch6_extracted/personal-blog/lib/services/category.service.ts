...
export const categoryService = {
  ...
  getAllWithRelations: async (userId: string): Promise<SerializedCategory[]> => {
    const items = await prisma.category.findMany({ where: { userId }, select: { id: true, name: true, createdAt: true, posts: { select: { id: true, title: true, excerpt: true, published: true } } }, orderBy: { createdAt: 'desc' }, take: 20, skip: 0 })
    return items.map(item => ({ ...item, createdAt: item.createdAt.toISOString() })) as SerializedCategory[]
  },

  getByIdWithRelations: async (userId: string, id: string): Promise<SerializedCategory> => {
    const item = await prisma.category.findFirst({ where: { id, userId }, select: { id: true, name: true, createdAt: true, posts: { select: { id: true, title: true, excerpt: true, published: true } } } })
    if (!item) notFound()
